...
$inputUrl = getopt('u:', ['url']) ? getopt('u:', ['url:']) : $argv[1];

if (!$inputUrl) {

    echo 'Please enter URL for validation';
    die;

} else if (is_array($inputUrl)) {

    if (array_key_exists('u', $inputUrl)) {

        $inputUrl = $inputUrl['u'];

    } else if (array_key_exists('url', $inputUrl)) {
...
function checkHost($url, $domains)
{
    ...
    } else $arrHost = $urlArr['path'];

    $testHost = explode('.', $arrHost);
    $testHostLength = count($testHost);
    $allowedDomains = explode(' ', $domains);
    $host = parse_url($url, PHP_URL_HOST);
    $domain = '';
    $subdomain = '';
    $tld = in_array(end($testHost), $allowedDomains);
    $sld = in_array(count($testHost) - 1, $allowedDomains);


    if ($testHostLength < 2) {

        echo 'Domain hierarchy is not correct';
        die;
        ...
        echo 'Top level domain is not correct';
        die;
    ...
    if ($sld && $sld !== $tld) {
        ...
        $sld = $testHost[$testHostLength - 2];

    } else {

        $sld = null;
        $getDomain = array_slice($testHost, $testHostLength - 2);
        $domain = implode('.', $getDomain);

    }

    if ($testHostLength > 2 && !$sld) {

        $getSubDomain = array_slice($testHost, 0,$testHostLength - 2);

        $subdomain = implode('.', $getSubDomain);
...
function getPath($url)
{
    ...
    if (array_key_exists('host', $url)) {
        $pathString = addcslashes($url['path'], '/');
        $extensionArray = explode('.', $url['path']);
...
{
    parse_str($query['query'], $parsedQuery);
...
$scheme = checkScheme($url, $uriSchemes);
$host = checkHost($url, $tldList);
$path = getPath($url);
$query = getQuery($url);
...
$parsedUrl = json_encode(array_merge($scheme, $host, $path, $query, $parsedQuery, $fragment), JSON_PRETTY_PRINT);
